...
def test_compare_quantopian():
    ...
    quant_slippage = -52.8     # https://www.quantopian.com/posts/test-with-slippage-to-compare-with-simple-back
    # within 10%/15% of both bounds
    assert abs(bt.summary.iloc[0]['Total Return (%) (Last Value)']-quant_no_slippage) <= 15
    assert abs(bt.summary.iloc[1]['Total Return (%) (Last Value)']-quant_slippage) <= 10